...
@Component({
    ...
    <app-book-navigator [current]="(current$ | async) + 1"
    [count]="count$ | async" (onNext)="nextBook($event)" (onPrevious)="previousBook($event)"></app-book-navigator>
    ...
    <table *ngIf="book">
      ...
      <td width="50%">
        ...
        <button class="add-cart" (click)="addBookToCart(book)"
        mat-raised-button color="primary">ADD TO CART <i class="material-icons">add_shopping_cart</i></button>
      </td>
    </table>
...
export class SelectedBookComponent implements OnInit {
    ...
    nextBook() {
        ...
        if (this.nextId) { this.router.navigate(['/catalog/book', this.nextId]); }
    ...
    previousBook() {
        ...
        if (this.previousId) { this.router.navigate(['/catalog/book', this.previousId]); }
    }
